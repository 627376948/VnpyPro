...
#                     ('rqdatad-pro.ricequant.com', 16011))
        try:
            ...
            ls = [x for x in self.db_client["quantaxis"]
                  ["stock_list"].find({})]
            df = pd.DataFrame(ls)
            for ix, row in df.iterrows():
                self.symbols.add(row['code'])
...
#         end += timedelta(1)
        if rq_interval == "1d":
            ...
            if rq_symbol[:2] == "15" or rq_symbol[:2] == "51":
                ls = [x for x in self.db_client["quantaxis"]["index_min"].find({"code": rq_symbol,
                                                                                "tyoe": rq_interval,
                                                                                "time_stamp": {"$gte": start, "$lte": end}})]
                df = pd.DataFrame(ls)
            else:
                ls = [x for x in self.db_client["quantaxis"]["stock_min"].find({"code": rq_symbol,
                                                                                "tyoe": rq_interval,
                                                                                "time_stamp": {"$gte": start, "$lte": end}})]
                df = pd.DataFrame(ls)
