...
class BasicMonitor(QtWidgets.QTableWidget):
    ...
    def saveToCsv(self):
        ...
        path, fileType = QtWidgets.QFileDialog.getSaveFileName(
            self, "保存数据", self.defaultPath, 'CSV(*.csv)')
    ...
    def saveToXls(self):
        ...
        self.menu.close()
        path, fileType = QtWidgets.QFileDialog.getSaveFileName(
            self, "保存数据", self.defaultPath, 'XLS(*.xls)')
        try:
            # if not path.isEmpty():
            if path:
...
class DataFrameMonitor(BasicMonitor):
    ...
    def update_one_data(self, data, is_lastrow=False):
    ...
    def on_event(self, event):
        ...
        try:
            ...
            self.initTable()
            for x in datalist:
                self.update_one_data(x, is_lastrow=True)
        except Exception as e:
